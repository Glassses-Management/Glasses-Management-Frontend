...
const inputClass =
  'w-full rounded-xl border border-gray-200 bg-white px-3.5 py-2.5 text-sm text-[#1a1a2e] outline-none transition-colors duration-200 placeholder:text-neutral-400 focus:border-[#8fa88f] dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100 dark:placeholder:text-neutral-500 dark:focus:border-[#8fa88f]'

function Field({ label, htmlFor, required, children }) {
  return (
    <div>
      <label htmlFor={htmlFor} className="mb-1.5 block text-sm font-medium text-[#1a1a2e] dark:text-neutral-200">
        {label}
        {required && <span className="ml-1 text-[#8fa88f]">*</span>}
      </label>
      {children}
    </div>
  )
...
function Badge({ tone, children }) {
  const tones = {
    green: 'bg-[#8fa88f]/15 text-[#6f8a6f]',
  ...
  return (
    <section id="inquiry" className="scroll-mt-20 py-16 md:py-20">
      <div className="mx-auto grid max-w-6xl gap-6 px-4 md:px-6 lg:grid-cols-2">
        {/* Inquiry form */}
        <div className="overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
          <div className="flex items-center gap-3 bg-gradient-to-r from-[#8fa88f] to-[#6f8a6f] px-6 py-5 text-white">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 p-6" noValidate>
            ...
            <button
              type="submit"
              disabled={sending}
              className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#8fa88f] px-5 py-3 text-sm font-medium text-white transition-colors hover:bg-[#6f8a6f] disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
            >
            ...
            <p className="flex items-start gap-2 text-xs leading-relaxed text-neutral-500 dark:text-neutral-400">
              <Lock size={13} className="mt-0.5 shrink-0 text-[#6f8a6f]" />
        ...
        {/* Schedule */}
        <div className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
            ...
            {LAB_NOTES.map(({ label, value }) => (
              <div key={label} className="flex items-center justify-between gap-3">
                <span className="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-400">
                  <Phone size={14} className="text-[#6f8a6f]" />
                  {label}
                </span>
          ...
          <div className="mt-6 rounded-xl bg-neutral-50 p-4 text-sm leading-relaxed text-neutral-600 dark:bg-neutral-900/40 dark:text-neutral-400">
            <p>
